...
namespace levin
{
    ...
    public:
        ...
        const glm::mat4 &matrix() const { return m_uniform_block.matrix; }
        glm::mat4 &matrix() { return m_uniform_block.matrix; }

        void flush(int current_frame)
        {
            // the camera should be sent to the uniform buffer
            UniformBlock block {};
            block.matrix = Camera::default_camera().matrix() * m_uniform_block.matrix;
        ...
        {
            m_descriptor_set.bind(command_buffer, pipeline, current_frame);
    ...
    public:
        ...
        void load_vertexes(const std::vector<Vertex> &vertexes);
