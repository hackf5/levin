...
void VulkanEngine::render(VkFramebuffer framebuffer)
{
    ...
    VkBuffer vertex_buffers[] = { (VkBuffer)*m_vertex_buffer };
    VkDeviceSize offsets[] = { 0 };
    vkCmdBindVertexBuffers(command_buffer, 0, 1, vertex_buffers, offsets);

    vkCmdBindIndexBuffer(command_buffer, (VkBuffer)*m_index_buffer, 0, VK_INDEX_TYPE_UINT16);

    vkCmdDrawIndexed(command_buffer, static_cast<uint32_t>(indexes.size()), 1, 0, 0, 0);

    vkCmdDraw(command_buffer, static_cast<uint32_t>(vertexes.size()), 1, 0, 0);
